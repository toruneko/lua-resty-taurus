...
our $HttpConfig = <<'_EOC_';
    ...
    init_by_lua_block {
        ...
        local text = io.open("$TEST_NGINX_CWD/t/rule/003-in_array-rule.yaml"):read("*a")
...
=== TEST 1: in array rule match demo1
...
=== TEST 2: in array rule match demo2
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
...
=== TEST 3: in array rule not match
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
...
=== TEST 4: in array rule not match, no parameter
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
